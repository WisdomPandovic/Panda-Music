...
function IgboRapMusic() {
  ...
  useEffect(() => {
    const fetchIgboRapTracks = async () => {
      try {
        ...
        const response = await fetch(
          'https://api.spotify.com/v1/search?q=igbo%20rap&type=track',
          {
            headers: {
        ...
        const tracksData = responseData.tracks.items;
            ...
            {tracks.length > 0 && (
              tracks.map(track => (
                <li key={track.id} className="track-item track" onClick={() => handleTrackClick(track)}>
                  ...
                  <strong>Artist(s):</strong> {track.artists.map(artist => artist.name).join(', ')}<br />
